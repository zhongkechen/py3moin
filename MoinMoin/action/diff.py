...
def execute(pagename, context):
    ...
    try:
        date = context.request.values['date']
        try:
    ...
    try:
        rev1 = int(context.request.values.get('rev1', -1))
    except Exception:
        rev1 = 0
    try:
        rev2 = int(context.request.values.get('rev2', 0))
    except Exception:
        rev2 = 0
    ...
    # spacing flag?
    ignorews = int(context.request.values.get('ignorews', 0))

    _ = context.getText

    # get a list of old revisions, and back out if none are available
    currentpage = Page(context, pagename)
    currentrev = currentpage.current_rev()
    if currentrev < 2:
        context.theme.add_msg(_("No older revisions available!"), "error")
    ...
    if rev1 == -1:
        oldrev = currentrev - 1
        oldpage = Page(context, pagename, rev=oldrev)
    elif rev1 == 0:
        oldrev = currentrev
        oldpage = currentpage
    else:
    ...
    if rev2 == 0:
        ...
        newrev = rev2
        newpage = Page(context, pagename, rev=newrev)
    ...
    if not oldlog or not newlog:
        ...
        context.theme.add_msg(_("No log entries found."), "error")
        currentpage.send_page()
        return
    ...
    context.theme.send_title(_('Diff for "%s"') % (pagename,), pagename=pagename, allow_doubleclick=1)

    f = context.formatter
    context.write(f.div(1, id="content"))
    ...
    page_url = wikiutil.escape(currentpage.url(context), True)

    def enabled(val):
        return not val and u' disabled="disabled"' or u''

    revert_html = ""
    if context.user.may.revert(pagename):
        revert_html = """
...
""" % {
    ...
    rev_info_old_html = rev_info_html % {
        ...
        'date': context.user.getFormattedDateTime(wikiutil.version2timestamp(oldlog.ed_time_usecs)) or _('N/A'),
    ...
    rev_info_new_html = rev_info_html % {
        ...
        'date': context.user.getFormattedDateTime(wikiutil.version2timestamp(newlog.ed_time_usecs)) or _('N/A'),
        'rev_comment': wikiutil.escape(newlog.comment) or '',
    }

    if context.user.show_fancy_diff:
        from MoinMoin.util import diff_html
        context.write(f.rawHTML(
            diff_html.diff(context, oldpage.get_raw_body(), newpage.get_raw_body(), old_top=rev_info_old_html,
        ...
        if not lines:
            ...
            if ignorews:
                context.write(f.text(_('(ignoring whitespace)')), f.linebreak())
            else:
                ...
                if rev2:
                    qstr['rev2'] = str(rev2)
                context.write(f.paragraph(1), Page(context, pagename).link_to(context,
                                                                              text=_(
                                                                                  'Ignore changes in the amount of whitespace'),
                                                                              querystr=qstr, rel='nofollow'),
                              f.paragraph(0))

            context.write(f.preformatted(1))
            for line in lines:
                if line[0] == "@":
                    context.write(f.rule(1))
                context.write(f.text(line + '\n'))
            context.write(f.preformatted(0))
