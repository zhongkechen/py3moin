...
class ActionBase:
    ...
    def __init__(self, pagename, context, only_form=False):
        ...
        if only_form:
            ...
            self.form = context.request.values
        self.cfg = context.cfg
        self._ = _ = context.getText
    ...
    def render(self):
        ...
        if self.is_excluded():
            ...
            if isinstance(allowed, tuple):
                ...
                msg = _('You are not allowed to use action %(actionname)s on this page!') % {
                    'actionname': self.actionname}
            if not allowed:
                error = msg
        if error is None:
        ...
        if error:
            ...
            if self.ticket_ok():
                ...
                success = False
                self.error = _('Please use the interactive user interface to use action %(actionname)s!') % {
                    'actionname': self.actionname}
...
def do_raw(pagename, context):
    ...
    if not context.user.may.read(pagename):
        ...
        mimetype = context.request.values.get('mimetype', None)
        if mimetype and not MIMETYPE_CRE.match(mimetype):
            mimetype = None
        Page(context, pagename, rev=rev).send_raw(mimetype=mimetype)
...
def get_names(config):
    ...
    if not hasattr(config.cache, 'action_names'):
        ...
        actions = set([action for action in actions if action not in config.actions_excluded])
...
def get_available_actions(config, page, user):
    ...
    return set([action for action in actions if action not in excluded])
