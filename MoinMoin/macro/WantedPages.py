...
def macro_WantedPages(macro):
    ...
    controlbar = macro.formatter.div(1, css_class="controlbar") + \
                 page.link_to(context, label, querystr={'allpages': '%s' % (allpages and '0' or '1')}) + \
                 macro.formatter.div(0)

    # Get page dict readable by current user
    pages = context.rootpage.getPageDict()
    ...
    for name, page in list(pages.items()):
        ...
        if not allpages and wikiutil.isSystemPage(context, name):
            continue
        ...
        for link in links:
            if link not in pages and context.user.may.read(link):
                if is_deprecated:
    ...
    for name in wantednames:
        ...
        wherelinks = [pages[pagename].link_to(context, querystr={'highlight': name}, rel='nofollow')
